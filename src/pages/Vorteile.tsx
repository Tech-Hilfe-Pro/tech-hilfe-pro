...
const Vorteile = () => {
  ...
  const handleConsultation = () => {
    if (typeof window !== 'undefined' && window.umami) {
      window.umami.track('cta_consultation', { location: slug ? `benefit_${slug}` : 'vorteile' });
    }
  };

  const handleWhatsApp = () => {
    if (typeof window !== 'undefined' && window.umami) {
      window.umami.track('whatsapp_start', { location: slug ? `benefit_${slug}` : 'vorteile' });
    }
    window.open("https://wa.me/4915565029989?text=Hallo,%20ich%20interessiere%20mich%20für%20ein%20Service-Paket...", "_blank");
  };

  // If we have a specific benefit slug, show the benefit detail page
  if (benefit) {
  ...
  // For invalid benefit slugs, show 404
  if (slug && !benefit) {
    return (
      <div className="min-h-screen bg-background">
        <Header />
        <main className="py-20">
          <div className="max-w-4xl mx-auto px-4 text-center">
            ...
            <Link to="/vorteile" className="btn-hero">
              Zu den Vorteilen
            </Link>
          </div>
        </main>
  ...
  // Default: Show the benefits overview page
  return (
    <div className="min-h-screen bg-background">
      <Header />
